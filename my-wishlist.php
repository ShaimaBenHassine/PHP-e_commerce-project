...
if(isset($_GET['del']))
{
$query=$conn->prepare("DELETE from wishlist where id='$wid'");
$query->execute();
}
...
if(isset($_GET['action']) && $_GET['action']=="add"){
	$id=intval($_GET['id']);
	$query=$conn->prepare("DELETE from wishlist where productId='$id'");
	$query->execute();
	if(isset($_SESSION['cart'][$id])){
		$_SESSION['cart'][$id]['quantity']++;
	}else{
		$query_p=$conn->prepare("SELECT * FROM products WHERE id={$id}");
		$query_p->execute();
...
<div class="body-content outer-top-bd">
	...
	<div class="table-responsive">
		<table class="table">
			...
			<tbody>
<?php
$ret=$conn->prepare("SELECT products.productName as pname,products.productName as proid,products.productImage1 as pimage,products.productPrice as pprice,wishlist.productId as pid,wishlist.id as wid from wishlist join products on products.id=wishlist.productId where wishlist.userId='".$_SESSION['id']."'");
$ret->execute();
$num=$ret->rowCount();
	if($num>0)
	{
...
<?php $rt=$conn->prepare("SELECT * from productreviews where productId='$pd'");
$rt->execute();
$num=$rt->rowCount();
{
?>
