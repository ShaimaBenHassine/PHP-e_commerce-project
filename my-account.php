...
header('location:login.php');
	...
	if(isset($_POST['update']))
	{
		...
		$query=$conn->prepare("UPDATE users set name='$name',contactno='$contactno' 
...
{
$sql=$conn->prepare("SELECT password FROM  users where password='".md5($_POST['cpass'])."' && id='".$_SESSION['id']."'");
$sql->execute();
$num=$sql->rowCount();
if($num>0)
{
 $ret=$conn->prepare("UPDATE users set password='".md5($_POST['newpass'])."', updationDate='$currentTime' where id='".$_SESSION['id']."'");
 $ret->execute();
