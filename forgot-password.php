...
$query=$conn->prepare("SELECT * FROM users WHERE email='$email' and contactno='$contact'");
$query->execute();
$num=$query->rowCount();
if($num>0)
{
$extra="forgot-password.php";
$sql=$conn->prepare("UPDATE users set password='$password' WHERE email='$email' and contactno='$contact' ");
$sql->execute();
